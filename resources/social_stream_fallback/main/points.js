...
class PointsSystem {
    ...
    createDefaultUserData(username, type = 'default') {
        ...
        return {
            ...
            lastActive: 0,
            engagementHistory: [],
            // Capped-elapsed chat-presence watch time, minutes. Honest v1: this only
            // accrues while the user is actively chatting (see recordEngagement below).
            // LURKER time (present but silent) is NOT tracked — that needs platform
            // viewer-list APIs, which are out of scope here. Never accrues while the
            // app is closed, since nothing calls recordEngagement then.
            watchtimeMinutes: 0
        };
    }
    ...
    async recordEngagement(username, type = 'default', timestamp = Date.now()) {
        ...
        return this.withUserLock(userKey, async () => {
            ...
            // Watch-time accrual: CAPPED-ELAPSED method, no new timers, reuses the
            // existing lastActive bookkeeping below. On every message (not just once
            // per engagement window, unlike points) credit the elapsed time since this
            // user's previous activity, capped at engagementWindow so a long gap (or
            // the app being closed) never counts as watched time. If lastActive is 0
            // (first-ever engagement) there is nothing to cap against, so no credit —
            // additive only, never touches points/streak/leaderboard shape.
            if (userData.lastActive > 0) {
                const elapsedSinceLastActiveMs = now - userData.lastActive;
                const cappedMs = Math.max(0, Math.min(elapsedSinceLastActiveMs, this.engagementWindow));
                userData.watchtimeMinutes = (userData.watchtimeMinutes || 0) + (cappedMs / POINTS_MS_PER_MINUTE);
            }
            ...
            index.openCursor(null, 'prev').onsuccess = event => {
                ...
                if (cursor && users.length < limit) {
                    ...
                    users.push({
                        ...
                        // Additive field (item C, watch-time loyalty spec): existing
                        // consumers (leaderboard.html, scoreboard.html) pick named
                        // fields off each entry and ignore unknowns, so this is safe.
...
// Botrix loyalty-export importer (watch-time loyalty spec, item 1B). Manual-trigger
// ONLY — nothing in points.js calls this automatically. Import shape:
// { _meta, viewers: [{ rank, name, platform, points, watchtimeMinutes,
//   watchtimeFormatted, level, xp }] } (see
// ~/dev/pacsarcade/overlays-import/botrix/loyalty-export-2026-08-17.json).
//
// userKey mapping (must match PointsSystem.getUserKey exactly):
//   getUserKey(username, type) => `${username}:${type || 'default'}`
// viewer.name -> username, viewer.platform ("kick") -> type — viewer.platform is
// already the lowercase platform string, matching the "kick" value SSN's Kick
// source sets on message.type. This is an EXACT string match only; no casing or
// fuzzy normalization is applied, so a display-name mismatch between the Botrix
// export and SSN's captured chatname silently skips that viewer (counted in
// `skipped`, visible per-row in `changes`) rather than guessing a merge.
//
// watchtimeMinutes is imported ONLY by default: watchtimeMinutes = max(existing,
// imported) — idempotent, never decreases real accrued data, safe to re-run.
//
// Botrix points/xp/level are Botrix-scale (millions) and are NEVER written into
// SSN's own `points` field (would corrupt the message-engagement leaderboard).
// They are only written at all behind the explicit, off-by-default
// {includePoints:true} option, and even then land in separate namespaced fields
// (botrixPoints/botrixXp/botrixLevel) as a snapshot overwrite of the latest
// export values — kept for reference/future use, not mixed into SSN points math.
//
// Dry-run: {dryRun:true} computes and returns the would-be changes, writes nothing.
async function importBotrixWatchtime(jsonInput, options = {}) {
    ...
    if (!data || !Array.isArray(data.viewers)) {
        return { success: false, message: 'Invalid Botrix export format: missing viewers array' };
    }
    ...
    for (const viewer of data.viewers) {
        if (!viewer || !viewer.name || !viewer.platform) {
            skipped++;
            continue;
        }

        const username = viewer.name;
        const type = viewer.platform;
        const importedWatchtimeMinutes = Number(viewer.watchtimeMinutes) || 0;

        const existing = await pointsSystem.getUserPoints(username, type);
        const existingWatchtimeMinutes = existing.watchtimeMinutes || 0;
        const nextWatchtimeMinutes = Math.max(existingWatchtimeMinutes, importedWatchtimeMinutes);
        const watchtimeWillChange = nextWatchtimeMinutes !== existingWatchtimeMinutes;

        const change = {
            userKey: existing.userKey,
            username,
            type,
            existingWatchtimeMinutes,
            importedWatchtimeMinutes,
            nextWatchtimeMinutes,
            watchtimeWillChange
        };

        let willWrite = watchtimeWillChange;

        if (includePoints) {
            change.existingBotrixPoints = existing.botrixPoints || 0;
            change.importedBotrixPoints = Number(viewer.points) || 0;
            change.existingBotrixXp = existing.botrixXp || 0;
            change.importedBotrixXp = Number(viewer.xp) || 0;
            change.existingBotrixLevel = existing.botrixLevel || 0;
            change.importedBotrixLevel = Number(viewer.level) || 0;
            change.botrixWillChange = (
                change.importedBotrixPoints !== change.existingBotrixPoints ||
                change.importedBotrixXp !== change.existingBotrixXp ||
                change.importedBotrixLevel !== change.existingBotrixLevel
            );
            willWrite = willWrite || change.botrixWillChange;
        }
        ...
        if (!dryRun && willWrite) {
            existing.watchtimeMinutes = nextWatchtimeMinutes;
            if (includePoints) {
                existing.botrixPoints = change.importedBotrixPoints;
                existing.botrixXp = change.importedBotrixXp;
                existing.botrixLevel = change.importedBotrixLevel;
            }
            await pointsSystem.saveUserPoints(existing);
        }
    }

    if (!dryRun && applied > 0) {
        schedulePointsLeaderboardBroadcast('botrix-import', { immediate: true });
    }
    ...
    return {
        ...
        dryRun,
        includePoints,
        totalViewers: data.viewers.length,
        applied,
        skipped,
        changes
    };
