...
class SpotifyIntegration {
	constructor() {
		...
		this.pendingCodeVerifier = null; // secretless/PKCE exchange (house patch) — optional
    ...
    async loadTokens() {
        try {
            if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local) {
                ...
                // Dedicated key: settings-sync drops unknown keys from the
                // settings object, so tokens stored there don't survive a
                // restart. This key does — prefer it when present.
                const tokenStore = await chrome.storage.local.get(['spotifyTokens']);
                if (tokenStore?.spotifyTokens?.refreshToken) {
                    this.accessToken = tokenStore.spotifyTokens.accessToken || this.accessToken;
                    this.refreshToken = tokenStore.spotifyTokens.refreshToken;
                    this.tokenExpiry = tokenStore.spotifyTokens.tokenExpiry || 0;
                    console.log('Loaded Spotify tokens from dedicated token store');
                }
            }
    ...
    async refreshAccessToken() {
        // Secretless/PKCE branch (house patch): a client secret is optional now — tokens
        // minted by the branded pacsarcade.org spotify-auth.html PKCE page never had one.
        // Only bail out if we're missing the refresh token or the client id entirely.
        if (!this.refreshToken || !this.settings.spotifyClientId?.textsetting) {
            return;
        }
        const clientId = this.settings.spotifyClientId.textsetting;
        const clientSecret = this.settings.spotifyClientSecret?.textsetting;
        ...
        try {
            ...
            if (hasSecret) {
                // Confidential-client fallback (existing/legacy setups).
                headers['Authorization'] = `Basic ${btoa(`${clientId}:${clientSecret}`)}`;
            } else {
                // Public-client PKCE refresh: no secret, no Basic header — client_id in body.
    ...
    async exchangeCodeForToken(code, redirectUri) {
        // Secretless/PKCE branch (house patch): if no client secret is configured, exchange
        // as a public client — client_id + code_verifier in the body, no Basic header. The
        // verifier comes from a PKCE-aware authorize step (e.g. a future buildAuthUrl using
        // code_challenge) stashed on this.pendingCodeVerifier; today's built-in click-to-auth
        // flow doesn't generate one, so this mainly documents the path — in production, PKCE
        // exchange happens on the branded spotify-auth.html page itself, which hands SSN
        // already-minted tokens via the existing postMessage/manual-paste contract (see
        // handleAuthCallback's callers) and never calls this function at all.
        const clientId = this.settings.spotifyClientId.textsetting;
        const clientSecret = this.settings.spotifyClientSecret?.textsetting;
        const hasSecret = !!clientSecret;
        ...
        try {
            const headers = { 'Content-Type': 'application/x-www-form-urlencoded' };
            let body = `grant_type=authorization_code&code=${code}&redirect_uri=${encodeURIComponent(redirectUri)}`;
            if (hasSecret) {
                headers['Authorization'] = `Basic ${btoa(`${clientId}:${clientSecret}`)}`;
            } else {
                body += `&client_id=${encodeURIComponent(clientId)}`;
                if (this.pendingCodeVerifier) {
                    body += `&code_verifier=${encodeURIComponent(this.pendingCodeVerifier)}`;
