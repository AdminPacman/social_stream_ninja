class EventFlowSystem {
	...
	replaceTemplateVars(text, message) {
		...
		return text.replace(/\{([\w.]+)\}/gi, (match, key) => {
			// Dot-path support: {webhookresponse.height} digs into object fields
			const path = key.split('.');
			let val = messageData[path[0].toLowerCase()];
			for (let i = 1; i < path.length && val !== undefined && val !== null; i++) {
				if (typeof val === 'object') {
					const seg = path[i];
					val = (seg in val) ? val[seg] : val[String(seg).toLowerCase()];
				} else {
					val = undefined;
				}
			}
					...
					if (config.syncMode) {
						...
						try {
							...
							if (!response.ok) {
								...
								if (parsed !== null) {
									// Bitcoin Federated Time enrichment: when a response carries a
									// block height, precompute BFT readings so templates can use
									// {bftheight} {bftclock} {bfttime} {bftdate} directly.
									// day=144 blocks, hour=6, year=13 months x 28 days = 364 days.
									let bft = {};
									const bftHeight = (parsed && typeof parsed === 'object')
										? Number(parsed.height !== undefined ? parsed.height : parsed.blockHeight)
										: NaN;
									if (Number.isFinite(bftHeight) && bftHeight >= 0) {
										const beat = bftHeight % 144;
										const hh = String(Math.floor(beat / 6)).padStart(2, '0');
										const mm = String((beat % 6) * 10).padStart(2, '0');
										// No :ss here — the honest-clock law. BFT seconds are
										// wall-seconds-since-last-block (needs the block's own
										// timestamp, which this context doesn't have); the wall
										// clock's getSeconds() is NOT block time and must never be
										// glued on. hh:mm is chain-exact from the height alone.
										const days = Math.floor(bftHeight / 144);
										const year = String(Math.floor(days / 364)).padStart(4, '0');
										const rem = days % 364;
										const month = String(Math.floor(rem / 28) + 1).padStart(2, '0');
										const day = String((rem % 28) + 1).padStart(2, '0');
										bft = {
											bftheight: bftHeight,
											bftclock: `${hh}:${mm}`,
											bfttime: `${hh}:${mm}`,
											bftdate: `${year}.${month}.${day} a₿`
										};
									}
									result.message = { ...message, ...bft, webhookResponse: parsed, webhookStatus: response.status };
								} else {
