...
(function () {
    ...
    // v3 (0018.05.26, Pac live-testing amendment): the Style panel's OWN
    // CLOCK segmented control + seconds toggle are REMOVED here — a separate
    // branch is moving clock control into a popover on the topbar clock
    // itself. The settings keys (CLOCK_MODE_KEY/CLOCK_SECONDS_KEY) and the
    // actual clock render/tick pipeline (loadClockSettings/saveClockSetting/
    // applyClockSettingChange, all defined near the top of this file) are
    // UNTOUCHED and still read/applied independently at boot (see the
    // loadClockSettings().then(...) call in the boot sequence) — only this
    // panel's now-dead widget wiring (initClockControls, formerly bound to
    // #arcade-clock-seg/#arcade-clock-seconds, both removed from
    // buildStylePanel()'s markup) is gone. syncClockControls() is left
    // defined (still called at boot) — it degrades to a safe no-op now that
    // those ids no longer exist in the DOM, same "no-op until it resolves,
    // but safe" contract it already had. COORDINATION NOTE FOR MERGE
    // SEQUENCING: land the topbar-clock-popover branch either before or
    // after this one — neither touches the other's code, only this panel's
    // now-absent CLOCK row (which the popover branch was never depending on).
